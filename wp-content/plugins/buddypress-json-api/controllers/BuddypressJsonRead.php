...
public function profile_delete_profile() {	
	...
	$userid = $_POST['userid'];
	$pw = $_POST['pw'];	
	if(aheadzen_check_valid_user($userid,$pw)){
		wp_set_current_user($userid);
		if(bp_core_delete_account($userid)){
  ...
  return $oReturn;
	...
	*/
	...
	public function activity_set_bpfb()
	{
		...
		$groupid = 0;
		if(!$activity_type){$activity_type = 'activity_photo';}
    ...
    public function activity_get_mentions() {
		...
		if($res){
			...
			foreach($res as $oMentions){
				...
				$oReturn->mentions[$counter]->content = do_shortcode($oMentions->content);
