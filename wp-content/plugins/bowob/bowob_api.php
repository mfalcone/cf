...
function bowob_api_get_code($app_id, $server_address)
{
  ...
  $output .= '<script type="text/javascript">';
    $output .= 'bowob_user_nick = "' . str_replace("\"", "\\\"", str_replace("\\", "\\\\", bowob_get_user_nick())) . '";';
  $output .= '</script>' . "\n";
  $output .= '<script type="text/javascript" src="' . $server_address . $app_id . '/bl_2/loader.js"></script>' . "\n";
  
  if(preg_match('#^([^:]+://)[^\.]+(.*)$#', $server_address, $match))
  {
    $rescue_address = $match[1] . 'rescue' . $match[2];
    $output .= '<script type="text/javascript" src="' . $rescue_address . $app_id . '/bl_2/loader.js"></script>' . "\n";
  }
