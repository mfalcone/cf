	...
	class BPGE_ADMIN_GENERAL extends BPGE_ADMIN_TAB {
		...
		public function register_sections() {
			...
			add_settings_field( 're_pages',
			                    __( 'Rich Editor for Pages', 'buddypress-groups-extras' ),
			                    array( $this, 'display_re_pages' ),
			                    $this->slug,
			                    $this->slug . '_settings' );
			add_settings_field( 're_fields',
			                    __( 'Rich Editor for Fields', 'buddypress-groups-extras' ),
			                    array( $this, 'display_re_fields' ),
			                    $this->slug,
			                    $this->slug . '_settings' );
			add_settings_field( 'access',
			                    __( 'User Access', 'buddypress-groups-extras' ),
			                    array( $this, 'display_access' ),
			                    $this->slug,
			                    $this->slug . '_settings' );
			add_settings_field( 'import',
			                    __( 'Data Import', 'buddypress-groups-extras' ),
			                    array( $this, 'display_import' ),
			                    $this->slug,
			                    $this->slug . '_settings' );
			add_settings_field( 'uninstall',
			                    __( 'Uninstall Options', 'buddypress-groups-extras' ),
		...
		public function display_access() {
			...
			<p><?php _e( 'Who can open group admin tab Extras?', 'buddypress-groups-extras' ); ?></p>
			<?php
		...
		public function display_import() { ?>
			<p>
				<?php _e( 'If you upgraded from any version of BuddyPress Groups Extras, which had the version number less than 3.4, and if you want to preserve all previously generated content (like default and groups fields etc) please do the import using controls below.', 'buddypress-groups-extras' ); ?>
			</p>

			<p class="description"><?php _e( '<strong>Important</strong>: Do not import data twice - as this will create lots of duplicated fields.', 'buddypress-groups-extras' ); ?></p>
		...
		public function display_re_pages() {
			...
			echo '<ul>';
			echo '<li><label><input type="radio" name="bpge_re" ' . ( $this->bpge['re'] == 1 ? 'checked="checked"' : '' ) . ' value="1">&nbsp' . __( 'Enable', 'buddypress-groups-extras' ) . '</label></li>';
			echo '<li><label><input type="radio" name="bpge_re" ' . ( $this->bpge['re'] != 1 ? 'checked="checked"' : '' ) . ' value="0">&nbsp' . __( 'Disable', 'buddypress-groups-extras' ) . '</label></li>';
			echo '</ul>';
		}
		...
		public function display_re_fields() {
			...
			echo '<ul>';
			echo '<li><label><input type="radio" name="bpge_re_fields" ' . checked( $this->bpge['re_fields'], 'yes', false ) . ' value="yes">&nbsp' . __( 'Enable', 'buddypress-groups-extras' ) . '</label></li>';
			echo '<li><label><input type="radio" name="bpge_re_fields" ' . checked( $this->bpge['re_fields'], 'no', false ) . ' value="no">&nbsp' . __( 'Disable', 'buddypress-groups-extras' ) . '</label></li>';
			echo '</ul>';
		}
		...
		public function display_uninstall() {
			...
			echo '<p>';
			echo '<label><input type="radio" name="bpge_uninstall" ' . ( $this->bpge['uninstall'] == 'no' ? 'checked="checked"' : '' ) . ' value="no">&nbsp' . __( 'Preserve all data', 'buddypress-groups-extras' ) . '</label><br />';
			echo '<label><input type="radio" name="bpge_uninstall" ' . ( $this->bpge['uninstall'] == 'yes' ? 'checked="checked"' : '' ) . ' value="yes">&nbsp' . __( 'Delete everything', 'buddypress-groups-extras' ) . '</label>';
			echo '</p>';
		}
