...
if ( ! class_exists( 'BPGE_ADMIN_SETS' ) ) {
	...
	class BPGE_ADMIN_SETS extends BPGE_ADMIN_TAB {
		...
		public function __construct() {
			$this->title = __( 'Default Sets of Fields', 'buddypress-groups-extras' );
		...
		public function display() {
			...
			_e( 'Please create/edit here fields you want to be available as standard blocks of data.<br />This will be helpful for group admins - no need for them to create lots of fields from scratch.', 'buddypress-groups-extras' );
			echo '</p>';
			...
			if ( ! empty( $set_of_fields ) ) {
				...
				echo '<li>';
				echo '<span class="no_fields">' . __( 'Currently there are no predefined fields. Groups admins should create all fields by themselves.', 'buddypress-groups-extras' ) . '</span>';
				echo '</li>';
			}
			echo '</ul>';
