...
function ac_notifier_format_notifications ( $action, $activity_id, $secondary_item_id, $total_items, $format = 'string' ) {
	...
	if ( $action == $ac_action ) {
		...
		if ( $count > 2 ) {
			...
			$glue = __(' y ', 'bp-activity-comment-notifier'); //if there are 2 unique users , say x and y commented
		}
		...
		if ( $self_post ) {
			...
			if ( $total_user <= 2) {
				$text = sprintf( __('%s comentó tu publicación', 'bp-activity-comment-notifier' ), $commenting_users );
			} else {
				$text = sprintf( __('%s y otros %s comentaron en tu publicación', 'bp-activity-comment-notifier'), $commenting_users, $count );
			}
			...
			if ( $total_user == 1) {
				if ( $activity->user_id == reset($users) ) {
					$text = sprintf( __('%s también comentó en su publicación', 'bp-activity-comment-notifier'), $commenting_users, bp_core_get_user_displayname( $activity->user_id ) );
				} else {
					$text = sprintf( __('%s también comentó en la publicación de %s', 'bp-activity-comment-notifier'), $commenting_users, bp_core_get_user_displayname( $activity->user_id ) );
				}
			}
			if ( $total_user == 2) {
				$text = sprintf( __('%s también comentó en la publicación de %s', 'bp-activity-comment-notifier'), $commenting_users, bp_core_get_user_displayname( $activity->user_id ) );
			} else {
				$text = sprintf( __('%s y %s otros también comentaron en la publicación de  %s', 'bp-activity-comment-notifier'), $commenting_users, $count, bp_core_get_user_displayname( $activity->user_id ) );
			}
		...
		$label = __( 'publicación', 'bp-activity-comment-notifier' );
		
		if ( $activity->type == 'activity-comment' ) {
			$label = __('comentario', 'bp-activity-comment-notifier' );
		}
		
		$name = bp_core_get_user_displayname( $secondary_item_id );
		$text = sprintf( __('%s guardó tu %s', 'bp-activity-comment-notifier'), $name, $label ); 
