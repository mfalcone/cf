 ...
 *
 */
function bp_like_add_user_like( $item_id, $type ,$activity_autor) {
    ...
    if ( bp_is_active( 'notifications' ) ) {
        bp_notifications_add_notification( array(
            'user_id'           => $activity_autor,
            'item_id'           => $item_id,
            'secondary_item_id' => $item_id->user_id,
            'component_name'    => 'custom',
            'component_action'  => 'custom_action'.$item_id,
...
function custom_format_buddypress_notifications( $action, $item_id, $secondary_item_id, $total_items, $format = 'string' ) {
    ...
    if ( 'custom_action'.$item_id === $action ) {
        $user_id = get_current_user_id();
        $users_who_like = bp_activity_get_meta( $item_id, 'liked_count', true );
        end($users_who_like);
        $key = key($users_who_like); 
        $activity  = bp_activity_get_permalink( $item_id );
        $nombredelchango = bp_core_get_username($key);
        $custom_title = 'titulo'.$arr;
        $custom_link  = $activity;//$activity;
        $custom_text = 'A '.$nombredelchango.' le gusta tu actividad';
        // WordPress Toolbar
        if ( 'string' === $format ) {
...
function custom_notifier_remove_notification ( $activity, $has_access ) {
    $bp = buddypress();
    ...
    if ( $has_access ) {//if user can view this activity, remove notification(just a safeguard for hidden activity)
        ...
        bp_notifications_mark_notifications_by_item_id( $user_id, $activity->id, 'custom', 'custom_action' . $activity->id );
       
    }   
}

add_action( 'bp_activity_screen_single_activity_permalink', 'custom_notifier_remove_notification', 10, 2 );
